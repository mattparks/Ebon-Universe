...
public class PlayerFocus implements IPlayer {
	...
	public void init() {
		...
		this.inputJump = new KeyButton(GLFW_KEY_SPACE);
		...
		focusEntity = EntityLoader.load("person").createEntity(Environment.getEntitys(), new Vector3f(position), new Vector3f(rotation));
	}
	...
	public void update(boolean paused) {
		if (!paused) {
			...
			if (inputJump.wasDown()) {
				velMove.y = JUMP_POWER;
			} else if (position.y >= 0.0f) {
				velMove.y -= FlounderEngine.getDelta();
			} else {
				velMove.y = 0.0f;
			}
