...
public class ShadowBox {
	...
	private void updateWidthsAndHeights(ICamera camera) {
		...
		farHeight = farWidth / FlounderEngine.getDevices().getDisplay().getAspectRatio();
		nearHeight = nearWidth / FlounderEngine.getDevices().getDisplay().getAspectRatio();
	}
	...
	private Matrix4f calculateCameraRotationMatrix(ICamera camera) {
		...
		Matrix4f.rotate(rotation, new Vector3f(0.0f, 1.0f, 0.0f), degreesToRadians(camera.getYaw()), rotation);
		Matrix4f.rotate(rotation, new Vector3f(1.0f, 0.0f, 0.0f), degreesToRadians(-camera.getPitch()), rotation);
		return rotation;
	}
