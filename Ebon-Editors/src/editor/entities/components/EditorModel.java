			...
			public void actionPerformed(ActionEvent ae) {
				...
				if (returnValue == JFileChooser.APPROVE_OPTION) {
					...
					if (selectedFile.contains("res/entities")) {
						...
						EditorEntities.instance.PATH_MODEL = new MyFile(MyFile.RES_FOLDER, "entities", filepath[filepath.length - 1].replace(".obj", ""), filepath[filepath.length - 1]);
					} else {
		...
		loadTexture.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				if (returnValue == JFileChooser.APPROVE_OPTION) {
					...
					if (selectedFile.contains("res/entities")) {
						String[] filepath = selectedFile.split("/");
						EditorEntities.instance.PATH_TEXTURE = new MyFile(MyFile.RES_FOLDER, "entities", filepath[filepath.length - 1].replace(".png", ""), filepath[filepath.length - 1]);
					} else {
						FlounderLogger.error("The selected texture path is not inside the res/entities folder!");
					}
				}
		...
		loadNormalMap.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				if (returnValue == JFileChooser.APPROVE_OPTION) {
					...
					if (selectedFile.contains("res/entities")) {
						String[] filepath = selectedFile.split("/");
						EditorEntities.instance.PATH_NORMALMAP = new MyFile(MyFile.RES_FOLDER, "entities", filepath[filepath.length - 1].replace("Normals.png", ""), filepath[filepath.length - 1]);
					} else {
						FlounderLogger.error("The selected texture path is not inside the res/entities folder!");
					}
				}
