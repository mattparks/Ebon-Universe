...
public class Model {
	...
	public Model(float[] vertices, float[] textureCoords, float[] normals, float[] tangents, float[] colours, int[] indices, int dimensions) {
		...
		FlounderLogger.error("indices = {" + Indices + "}");

		String Vertices = "";
		for (int i = 0; i < vertices.length; i++) {
			Vertices += ", " + vertices[i];
		}
		FlounderLogger.error("vertices = {" + Vertices + "}");

		String TextureCoords = "";
		for (int i = 0; i < textureCoords.length; i++) {
			TextureCoords += ", " + textureCoords[i];
		}
		FlounderLogger.error("textureCoords = {" + TextureCoords + "}");

		String Normals = "";
		for (int i = 0; i < normals.length; i++) {
			Normals += ", " + normals[i];
		}
		FlounderLogger.error("normals = {" + Normals + "}");

		String Tangents = "";
		for (int i = 0; i < tangents.length; i++) {
			Tangents += ", " + tangents[i];
		}
		FlounderLogger.error("tangents = {" + Tangents + "}");
	}
